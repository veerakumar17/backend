...
def fetch_flood_alert(city: str) -> float:
    """Returns cumulative 24h rainfall (mm) from forecast — used for flood trigger."""
    if not OPENWEATHER_API_KEY:
        raise ValueError("OPENWEATHER_API_KEY is not set in .env")
    ...
    response = requests.get(FORECAST_URL, params={
        ...
        "appid": OPENWEATHER_API_KEY,
        "units": "metric",
        "cnt": 8,  # 8 x 3h slots = 24 hours
    }, timeout=10)

    if response.status_code == 404:
        raise ValueError(f"City '{city}' not found in OpenWeatherMap")
    if response.status_code != 200:
        raise ConnectionError(f"OpenWeatherMap forecast API error: {response.status_code}")
